...
def fetch_and_save_img(word):
    data_dir = 'data/{}/'.format(word.replace(' ', '_'))
...
def img_url_list(word):
    ...
    """
    url = 'http://image.search.yahoo.co.jp/search?n=160&p={}&search.x=1'.format(
        quote(word))
    byte_content, _ = fetcher.fetch(url)
    structured_page = BeautifulSoup(
        byte_content.decode('UTF-8'), 'html.parser')
    img_link_elems = structured_page.find_all(
        'a', attrs={'target': 'imagewin'})
    img_urls = [
        e.get('href') for e in img_link_elems
        if e.get('href').startswith('http')
    ]
